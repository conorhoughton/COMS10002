...
algorithms is difficult to assess abstractly, optmization algorithms
...
components, or, for a particle physicist, a configuation of fields
...
the log-liklihood, a measure of how well a stastical model predicts
...
so that optimation algorithms end up finding these and missing the
global minimum. This isn't a problem we are going to discuss here.

\subsubsection*{Some optimatization algorithms}

Obivously we know the functional form of $E$ we can find its minimum
...
neuroscience examples, the model might take a while to
...
regular in general, the distances between the vertices need not be the
same, but there is required that it isn't flat: if you remove one
...
doesn't involve renumber, that is just done here for notational
convenience. The idea generally is to replace $\mathbf{x}_{n+1}$. It
is useful to define the center of the remaining points:
\begin{equation}
...
\end{equation}
Often this will be a sort of reasonable point so that if it replaced
...
\end{equation}
and then the algorithm progresses by working out the new order of the
points and repeating. This is called reflection and is shown in
Fig.\ref{fig:reflection}.

Reflection is the most common case, but there are others. The
reflected point $x_r$ might be better all of the existing points,
so
\begin{equation}
...
\caption{This illustates reflection \textbf{(a)} and expansion
...
Another possibility is that the reflection
point would be the worst point
\begin{equation}
...
are lower values of the objective function. As such a new point is
defined, the contraction point
\begin{equation}
...
Obviously after any of these moves the points are reordered and the
